...
//                        'width' => '100px',
                    ...
                    ['class' => 'yii\grid\CheckboxColumn',
                        'visible' => function($model) {
                            return $model->status; //($model->status == StudentExamResult::STATUS_DRAFT OR $model->status == StudentExamResult::STATUS_VERIFIED) ? TRUE : FALSE;
                        }
                    ],
                    ['class' => 'yii\grid\ActionColumn',
                        'visible' => '($model->status == StudentExamResult::STATUS_DRAFT OR $model->status == StudentExamResult::STATUS_VERIFIED) ? TRUE : FALSE',
                        'template' => '{update}',
                        'buttons' => [
                            'update' => function ($url, $model) {
                                return Html::a('<span class="glyphicon glyphicon-pencil"></span>', Yii::$app->urlManager->createUrl(['/institution/student-exams-result-update', 'id' => $model->student_exam_result_id, 'url' => 'indexpending', 'edit' => 't']), ['title' => Yii::t('yii', 'Edit/Update'),]
                                );
                            },
//                            'delete' => function ($url, $model) {
//                                return Html::a('<span class="glyphicon glyphicon-trash"></span>', Yii::$app->urlManager->createUrl(['/institution/student-exam-result/delete', 'id' => $model->student_exam_result_id, 'edit' => 't']), ['title' => Yii::t('yii', 'Delete'),]
//                                );
//                            }
                                ],
                            ],
                    ...
                    <div class="text-right">
                        <?= Html::submitButton('Submit Selected Results to HESLB', ['class' => 'btn btn-warning', 'confirm' => 'Are you sure you to perform this Operation']); ?>
                    </div>
                    <?= Html::endForm(); ?>
        </div>
    </div>
