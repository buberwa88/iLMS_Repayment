...
public static function getCriteriaFieldQuestion($type){
       return Yii::$app->db->createCommand("SELECT * from criteria_field  WHERE `type`='{$type}' AND parent_id IS NULL")->queryAll();  
  }
 public function TestApplicantCriteriaFieldAnswer($source_table,$source_table_value_field,$response_value,$operator,$applicationId)
    ...
    $values=Yii::$app->db->createCommand("SELECT count(*) as count FROM $source_table where $source_table_value_field $operator '{$response_value}' AND application_id='{$applicationId}'")->queryScalar();
        return $values;
    }
