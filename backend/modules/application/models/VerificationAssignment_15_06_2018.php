...
class VerificationAssignment extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['assignee', 'study_level', 'total_applications', 'assigned_by', 'created_at'], 'required'],
    ...
    public static function getUnverifiedApplication($applicant_category_id,$totalApplications) {
        $applicationDetails = Application::findBySql("SELECT * FROM application WHERE applicant_category_id='" . $applicant_category_id . "' AND (assignee ='' OR assignee IS NULL) ORDER BY application_id ASC LIMIT $totalApplications")->all();
