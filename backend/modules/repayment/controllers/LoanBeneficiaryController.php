...
class LoanBeneficiaryController extends Controller {
    ...
    public function actionRecalculateLoan($id) {
        $applicant_id = \yii\bootstrap\Html::encode($id);
        if ($this->reprocessBeneficiaryLoan($applicant_id) == FALSE) {
            $sms = 'Operation Failed: No approved repayment for this beneficiary';
            Yii::$app->session->setFlash('failure', $sms);
        }
        return $this->redirect(['/repayment/loan-beneficiary/view-loanee-details', 'id' => $applicant_id]);
    }
    ...
    /*
     * this function is used to reprocess/recalculate beneficiary loan one at a time
     */
        ...
        if ($has_repayments) {
            ...
            if ($totalPRC != $totalDisbursed) {
                ...
                foreach ($getDistinctAccademicYrPerApplicant as $resultsApp) {
                    ...
                    //------check if exist in the loan summary before -------
                    $checkExistsStatus = \backend\modules\repayment\models\LoanSummaryDetail::checkPRCexistPerAcademicYearInfirstLoanSummary($applicant_id, $PRC_id, $academicYearID);
                    $activeLoanSummary = \backend\modules\repayment\models\LoanSummaryDetail::getActiveLoanSummaryOfBeneficiary($applicant_id, $loan_give_to);
                    $loan_summary_id = $activeLoanSummary->loan_summary_id;
                    $lastLoanSummaryAcademicYearID = $activeLoanSummary->academic_year_id;
                    if ($checkExistsStatus->loan_summary_id > 0) {
                        if ($missingAmount > 0) {
                ...
                ####################################end for ORGIN####################################
                ########################here for amount paid in each loan repayment item#################################
                $AmountPaidPerItemTotalLAF = \frontend\modules\repayment\models\LoanRepaymentDetail::getAmountPaidPerItemtoBeneficiary($applicant_id, $LAF_id, $loan_give_to);
                $totalAmountAlreadyPaidLAF = $AmountPaidPerItemTotalLAF->amount;
                $AmountPaidPerItemTotalPNT = \frontend\modules\repayment\models\LoanRepaymentDetail::getAmountPaidPerItemtoBeneficiary($applicant_id, $PNT_id, $loan_give_to);
                $totalAmountAlreadyPaidPNT = $AmountPaidPerItemTotalPNT->amount;
                $AmountPaidPerItemTotalVRF = \frontend\modules\repayment\models\LoanRepaymentDetail::getAmountPaidPerItemtoBeneficiary($applicant_id, $VRF_id, $loan_give_to);
                $totalAmountAlreadyPaidVRF = $AmountPaidPerItemTotalVRF->amount;
                $AmountPaidPerItemTotalPRC = \frontend\modules\repayment\models\LoanRepaymentDetail::getAmountPaidPerItemtoBeneficiary($applicant_id, $PRC_id, $loan_give_to);
                $totalAmountAlreadyPaidPRC = $AmountPaidPerItemTotalPRC->amount;
                #########################end for amount paid in each loan repayment item######################
                #######################LAST LOAN SUMMARY get amount remained##########################################
                $activeLoanSummary = \backend\modules\repayment\models\LoanSummaryDetail::getActiveLoanSummaryOfBeneficiary($applicant_id, $loan_give_to);
                $loan_summary_id = $activeLoanSummary->loan_summary_id;
                $lastLoanSummaryAcademicYearID = $activeLoanSummary->academic_year_id;
                $vrfAmountLastLoanSummaryUnpaid = \frontend\modules\repayment\models\EmployedBeneficiary::getIndividualEmployeesVRFUnderBill($applicant_id, $loan_summary_id, $loan_give_to);
                $LAFAmountLoanSummaryUnpaid = \frontend\modules\repayment\models\EmployedBeneficiary::getIndividualEmployeesLAFUnderBill($applicant_id, $loan_summary_id, $loan_give_to);
                $penaltyAmountLastLoanSummaryUnpaid = \frontend\modules\repayment\models\EmployedBeneficiary::getIndividualEmployeesPenaltyUnderBill($applicant_id, $loan_summary_id, $loan_give_to);
                $PRCAmountLastLoanSummaryUnpaid = \frontend\modules\repayment\models\EmployedBeneficiary::getOutstandingPrincipalLoanUnderBill($applicant_id, $loan_summary_id, $loan_give_to);
                ###############################################################################################
                if ((($totalLAFORGN - $totalAmountAlreadyPaidLAF) - $LAFAmountLoanSummaryUnpaid) > 0) {
                    $generalMissingAmountLAF = ($totalLAFORGN - $totalAmountAlreadyPaidLAF) - $LAFAmountLoanSummaryUnpaid;
                    \backend\modules\repayment\models\LoanSummaryDetail::checkAmountMissingUpdateGeneral($loan_summary_id, $applicant_id, $generalMissingAmountLAF, $LAF_id);
...
//            }
    ...
    function actionConfirmNonbeneficiary($id) {
        $id = \yii\bootstrap\Html::encode($id);
        $model = LoanBeneficiary::find()->where(['applicant_id' => $id])->one();
        $disbursment = \backend\modules\repayment\models\LoanNonBeneficiary::hasDisbursement($id);

        if (!$model && !$disbursment) {
            try {
                //updating the current non beneficiary table;
                $update = LoanNonBeneficiary::updateAll(['is_active' => LoanNonBeneficiary::IN_ACTIVE], ['applicant_id' => $id]);
                $loan_non_beneficiary = new LoanNonBeneficiary();
                $loan_non_beneficiary->applicant_id = $id;
                $loan_non_beneficiary->created_by = \Yii::$app->user->id;
                $loan_non_beneficiary->user_comments = '';
                $loan_non_beneficiary->is_beneficiary = LoanNonBeneficiary::APPLICANT_BENEFICIARY;
                $loan_non_beneficiary->is_active = LoanNonBeneficiary::ACTIVE;
                if ($loan_non_beneficiary->save()) {
                    $sms = 'Operation successful, Applicant Confirmated Non-Beneficiary';
                    Yii::$app->session->setFlash('success', $sms);
                } else {
                    $sms = 'Operation failed, pleae try again later';
                    Yii::$app->session->setFlash('failure', $sms);
                }
            } catch (yii\db\Exception $e) {
                $log = \yii\log\Logger::
                        $sms = 'Operation failed, please try again later';
                Yii::$app->session->setFlash('failure', $sms);
            }
        } else {
            $sms = 'Operation failed, Applicant has Disbursment or Exists in Beneficiary List Please check';
            Yii::$app->session->setFlash('failure', $sms);
        }
        return $this->redirect(['/repayment/loan-beneficiary/view-loanee-details', 'id' => $id]);
    }
