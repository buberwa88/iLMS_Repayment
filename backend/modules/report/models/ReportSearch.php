...
class ReportSearch extends Report
{
    ...
    public function rules()
    {
        return [
            [['id', 'category'], 'integer'],
            [['name', 'file_name', 'field1', 'field2', 'field3', 'field4', 'field5', 'type1', 'type2', 'type3', 'type4', 'type5', 'description1', 'description2', 'description3', 'description4', 'description5', 'sql', 'sql_where', 'sql_order', 'sql_group', 'column1', 'column2', 'column3', 'column4', 'column5', 'condition1', 'condition2', 'condition3', 'condition4', 'condition5', 'package','item_name'], 'safe'],
        ];
    }
    ...
    public function searchAllReportFilter($params)
    {
        $user_id=Yii::$app->user->identity->user_id;
        $results_userRole=self::findBySql('SELECT item_name FROM auth_assignment WHERE user_id="'.$user_id.'"')->one();
        $role=$results_userRole->item_name;
        
        $query = Report::find()->where(['report_access.user_role'=>$role]);

        // add conditions that should always apply here
        $query->joinWith("reportAccess");
        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);
