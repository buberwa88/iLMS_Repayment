...
<br/><br/>
     <table width='100%'>
         <tr><th <?php echo $style5HeaderAmount; ?>>SNO</th><th <?php echo $style5HeaderAmount; ?>>Pay Period</th><th <?php echo $style5HeaderAmount; ?>>Amount</th><th <?php echo $style5HeaderAmount; ?>>Laf Portion</th><th <?php echo $style5HeaderAmount; ?>>Penalty Portion</th><th <?php echo $style5HeaderAmount; ?>>Vrf Portion</th><th <?php echo $style5HeaderAmount; ?>>Principal Portion</th><th <?php echo $style5HeaderAmount; ?>>Total Vrf</th><th <?php echo $style5HeaderAmount; ?>>Vrf Applicable</th><th <?php echo $style5HeaderAmount; ?>>Outstanding Vrf</th><th <?php echo $style5HeaderAmount; ?>>Oustanding Principal</th></tr>
...
//end  check loan repayment item balances       
        ...
        //foreach ($details_applicant as $paymentCalculation) { 			   
		   ...
		   if($totalOutstandingAmount > 0){
			...
			//CHECK FOR VRF ACRUE DAILY FIRST
			for($countFirst=1;$countFirst <= $totalMonths; ++$countFirst){
			$amountPTotalFirst +=$amount1;
			if($totalOutstandingAmount >= $amountPTotalFirst){
			//get payment date
			    $dateConstantForPayment=$constantPaymentDate;
				$dateCreatedqqFirst=date_create($dateConstantForPayment);
				$dateDurationAndTypeqqFirst=$countForPaymentDateFirst." ".$duration_type;
				date_add($dateCreatedqqFirst,date_interval_create_from_date_string($dateDurationAndTypeqqFirst));
				$payment_dateFirst=date_format($dateCreatedqqFirst,"Y-m-d");
				//end				
			$amountPTotalAccumulatedFirst=$amountPTotalFirst;
			$payment_dateLast_fromFirst=$payment_dateFirst;
			//distribute amount in respective item according to the repayment rate
			//----here for LAF portion----
			if($balanceLAFfirst > 0){
				if($amount1 >=$balanceLAFfirst ){
				$LAFportionFirst=$balanceLAFfirst;
                $remainingAmountFirst=$amount1-$balanceLAFfirst;			
				}else{
				$LAFportionFirst=$amount1;
                $remainingAmountFirst=0;				
				}
			}else{
			$remainingAmountFirst=$amount1;	
			}
			$totalLAFPaymentTotalFisrt=$totalLAFLoopFirst;
			$totalLAFLoopFirst +=$LAFportionFirst;
			if($balanceLAFfirst >= $totalLAFLoopFirst){$LAFportionFirst=$LAFportionFirst;}else{
				$LAFportion1First=$balanceLAFfirst - $totalLAFPaymentTotalFisrt;
				if($LAFportion1First > 0){
				$LAFportionFirst=$LAFportion1First;	
				}else{
					$LAFportionFirst=0;
				}
				}
			
			$amount_remainedFirst=$amount1-$LAFportionFirst;
			
			//----here for penalty portion----	
         $penalty_portionFirst = $amount_remainedFirst * $PNTRepaymentRate;
            if(($balancePNTFirst >= $penalty_portionFirst) && $balancePNTFirst > 0){
             $penalty_portionFirst=$penalty_portionFirst;   
            }else if((($balancePNTFirst < $penalty_portionFirst) && $balancePNTFirst > 0)){
             $penalty_portionFirst=$balancePNTFirst;   
            }else{
             $penalty_portionFirst=0;   
            }
		 
			$totalPNTPaymentTotalFirst=$totalPNTLoopFirst;
			$totalPNTLoopFirst +=$penalty_portionFirst;
			if($balancePNTFirst >= $totalPNTLoopFirst){$penalty_portionFirst=$penalty_portionFirst;}else{
				$penalty_portion1First=$balancePNTFirst - $totalPNTPaymentTotalFirst;
				if($penalty_portion1First > 0){
				$penalty_portionFirst=$penalty_portion1First;	
				}else{
					$penalty_portionFirst=0;
				}
				}
		$amount_remained1First=$amount_remainedFirst-$penalty_portionFirst;		
        //---end for penalty----
		//-----here for VRF portion----
        if($balancePRCFirst > 0){
         $vrf_portionFirst=$amount_remained1First * $vrfRepaymentRate; 
         if($balanceVRFfirst >=$vrf_portionFirst){
         $vrfTopayFirst=$vrf_portionFirst; 
         $amount_remained22First=$amount_remained1First-$vrfTopayFirst;
         }else{
         $vrfTopayFirst=$balanceVRFfirst; 
         $amount_remained22First=$amount_remained1First-$vrfTopayFirst;
         }         
        }else{
            if($balanceVRFfirst >=$amount_remained1First){
         $vrfTopayFirst=$amount_remained1First;
         $amount_remained22First=0; 
         }else{
         $vrfTopayFirst=$balanceVRFfirst; 
         $amount_remained22First=0;
         }
		...
		$totalVRFPaymentTotalFirst=$totalVRFLoopFirst;
			$totalVRFLoopFirst +=$vrfTopayFirst;
			if($balanceVRFfirst >= $totalVRFLoopFirst){$vrfTopayFirst=$vrfTopayFirst;}else{
				$vrfTopay1First=$balanceVRFfirst - $totalVRFPaymentTotalFirst;
				if($vrfTopay1First > 0){
				$vrfTopayFirst=$vrfTopay1First;	
				}else{
					$vrfTopayFirst=0;
				}
				}
		$amount_remained2First=$amount_remained1First-$vrfTopayFirst;		
		//end here for VRF portion----
		
		//check if principal amount exceed
        if($balancePRCFirst >= $amount_remained2First){
        $amount_remained2First=$amount_remained2First;    
        }else if($balancePRCFirst < $amount_remained2First && $balancePRCFirst >'0'){
        $amount_remained2First=$balancePRCFirst;    
        }else{
        $amount_remained2First='0';    
        }
		$totalPRCPaymentTotalFirst=$totalPRCLoopFirst;
			$totalPRCLoopFirst +=$amount_remained2First;
			if($balancePRCFirst >= $totalPRCLoopFirst){$amount_remained2First=$amount_remained2First;}else{
				$amount_remained211First=$balancePRCFirst - $totalPRCPaymentTotalFirst;
				if($amount_remained211First > 0){
				$amount_remained2First=$amount_remained211First;	
				}else{
					$amount_remained2First=0;
				}
				}
        // end check principle amount exceed
			//end
			$totalPrinciplePaidFirst +=$amount_remained2First;
			$principleBalanceFirst=$balancePRCFirst-$totalPrinciplePaidFirst;
			$principleBalancePreviousFirst=$balancePRCFirst-$totalPRCPaymentTotalFirst;
			
			$overallLAFinPaymentFirst +=$LAFportionFirst;
			$overallPNTinPaymentFirst +=$penalty_portionFirst;
			$overallVRFinPaymentFirst +=$vrfTopayFirst;
			}
			...
			$totalNumberOfDaysFirst=round((strtotime($payment_dateFirst)-strtotime($payment_date1First))/(60*60*24));
			...
			$balanceVRFfirst +=$totalAcruedVRF1First;
			++$countForPaymentDateFirst;
		}
		//get payment date
			    $dateConstantForPayment1FinalFirst=$payment_dateFirst;
				$dateCreatedqq1FinalFirst=date_create($dateConstantForPayment1FinalFirst);
				$dateDurationAndTypeqqFirst="1"." ".$duration_type;
				date_add($dateCreatedqq1FinalFirst,date_interval_create_from_date_string($dateDurationAndTypeqqFirst));
				$payment_date1FinalFirst=date_format($dateCreatedqq1FinalFirst,"Y-m-d");
				$totalNumberOfDays1FinalFirst=round((strtotime($payment_date1FinalFirst)-strtotime($payment_dateFirst))/(60*60*24));
			    $totalAcruedVRF1FinalFirst=$principleBalanceFirst*$vrfRepaymentRate*$totalNumberOfDays1FinalFirst/$numberOfDaysPerYear;
				$totalAcruedVRFfirst +=$totalAcruedVRF1FinalFirst;
				$balanceVRFfirst +=$totalAcruedVRF1FinalFirst;
				//end
			//END CHECK FOR VRF ACRUE DAILY
			

			$totalMonths=($balancePRC + $balanceLAF + $balancePNT + $balanceVRFfirst)/$amount1;
			$totalOutstandingAmount=($balancePRC + $balanceLAF + $balancePNT + $balanceVRFfirst);
			$balanceVRF=$balanceVRFfirst;
		...
		$totalVRFPaymentTotal=$totalVRFLoop;
			...
			if($balanceVRF >= $totalVRFLoop){$vrfTopay=$vrfTopay;}else{
	...
	<td <?php echo $style5DataAmount; ?>><?php echo number_format(($balanceVRF),2); ?></td>
    ...
    <td <?php echo $style5DataAmount; ?>><?php echo number_format(($principleBalance),2); ?></td>
        ...
        if($OverallBalance > 0 || ($balanceLAF > $overallLAFinPayment) || ($balancePNT > $overallPNTinPayment) || ($balanceVRF > $overallVRFinPayment) || ($balancePRC > $totalPrinciplePaid)){
			    //get payment date
...
<tr>
    ...
    <td <?php echo $style5DataAmount; ?>><?php echo number_format(($totalOutstandingAmount),2); ?></td>
