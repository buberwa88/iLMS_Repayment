...
class LoanSummary extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['employer_id', 'applicant_id', 'created_by', 'bill_status'], 'integer'],
            //[['bill_number', 'amount', 'created_at', 'created_by'], 'required'],
            [['amount'], 'number'],
            [['created_at', 'bill_status', 'employer_name', 'Employer_Code', 'Bill_Ref_No', 'traced_by', 'number_of_employees', 'dateSubmitted', 
            ...
            [['bill_number'], 'unique', 'message' =>'Bill number already exist'],
    ...
    public function getLastBillIDApplicant($applicantID){
      $details_bill_number = $this->findBySql("SELECT bill_number FROM loan_summary WHERE  applicant_id='$applicantID' AND bill_number LIKE 'BEN%' ORDER BY loan_summary_id DESC")->one();
      $bill=$details_bill_number->bill_number;     
    ...
    public function getActiveBill($employerID){
      $details_bill = $this->findBySql("SELECT * FROM loan_summary WHERE  employer_id='$employerID' AND (bill_status='0' OR bill_status='1') ORDER BY loan_summary_id DESC")->one();
      $bill=$details_bill->loan_summary_id;     
      $billValue = (count($bill) == 0) ? '0' : $details_bill;
        return $billValue;
        }
    public function getActiveBillLoanee($applicantID){
      $details_bill = $this->findBySql("SELECT * FROM loan_summary WHERE  applicant_id='$applicantID' AND (bill_status='0' OR bill_status='1') ORDER BY loan_summary_id DESC")->one();
      $bill=$details_bill->loan_summary_id;     
      $billValue = (count($bill) == 0) ? '0' : $details_bill;
        return $billValue;
        }
...
public function getTotalPaidunderBillIndividualEmployee($LoanSummaryID,$applicantID){
 ...
 public function getActiveBillToUpdateVRFofEmployees($employerID){
    ...
    $this->updateAll(['bill_status' =>'5'], 'loan_summary_id ="'.$LoanSummaryID.'" AND employer_id="'.$employerID.'"');
 }
    public function ceaseBillIfNewEmployeeAdded($employerID){
    $this->updateAll(['bill_status' =>'5'], 'employer_id="'.$employerID.'" AND (bill_status="0" OR bill_status="1")');
 }
    public function insertBillRequestApplicant($bill_status,$applicantID,$bill_number,$created_by,$created_at,$amount){
 ...
 public function getBillRequestedPending($applicantID){
      $details_bill = $this->findBySql("SELECT * FROM loan_summary WHERE  applicant_id='$applicantID' AND bill_status='7' ORDER BY loan_summary_id DESC")->one();
      $bill=$details_bill->loan_summary_id;     
      $billValue = (count($bill) == 0) ? '0' : $details_bill;
        return $billValue;
        }
