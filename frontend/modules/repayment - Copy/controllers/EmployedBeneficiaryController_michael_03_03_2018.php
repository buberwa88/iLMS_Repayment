...
class EmployedBeneficiaryController extends Controller {
    ...
    public function actionIndex() {
        ...
        if ($modelHeader->load(Yii::$app->request->post())) {
            ...
            if (strcmp($highestColumn, "N") == 0 && $highestRow >= 4) {
                ...
                $objPHPExcelOutput->getActiveSheet()->SetCellValue('A1', 'LOAN BENEFICIARIES EMPLOYEES UPLOAD REPORT');
                $objPHPExcelOutput->setActiveSheetIndex(0)->mergeCells('A1:O1', 'LOAN BENEFICIARIES EMPLOYEES UPLOAD REPORT');

                $rowCount = 2;
                $customTitle = ['SNo', 'CHECK/PF NUMBER', 'FORM FOUR INDEX NUMBER', 'FIRST NAME', 'MIDDLE NAME', 'SURNAME', 'DATE OF BIRTH(Year-Month-Day)', 'PLACE OF BIRTH(WARD)', 'MOBILE PHONE NUMBER', 'CURRENT NAME IF CHANGED', 'NAME OF INSTITUTION OF STUDY', 'NATIONAL IDENTIFICATION NUMBER(NIN)', 'BASIC SALARY(TZS)', 'GENDER', 'UPLOAD STATUS', 'FAILED REASON'];
                ...
                for ($row = 4; $row <= $highestRow; ++$row) {
                    ...
                    if ($sn != '' && $eployee_exists_status == 0 && $eployee_exists_nonApplicant == 0) {
                        if ($modelHeader->save()) {
                           ...
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('B' . $rowCount, $employee_id);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('C' . $rowCount, $f4indexno);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('D' . $rowCount, $firstname);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('E' . $rowCount, $middlename);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('F' . $rowCount, $surname);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('G' . $rowCount, $date_of_birth);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('H' . $rowCount, $place_of_birth);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('I' . $rowCount, $phone_number);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('J' . $rowCount, $employment_status);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('K' . $rowCount, $learning_institution_id);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('L' . $rowCount, $NID);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('M' . $rowCount, $basic_salary);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('N' . $rowCount, $sex);
                           $objPHPExcelOutput->getActiveSheet()->SetCellValue('O' . $rowCount, 'UPLOADE SUCCESSFUL');
    ...
    public function actionCreate() {
        ...
        if($modelEmployedBeneficiary->load(Yii::$app->request->post())){
            echo '<pre>';print_r($modelEmployedBeneficiary->attributes);
           $modelEmployedBeneficiary->validate();
           echo '<pre>';print_r($modelEmployedBeneficiary->errors);die('Michael...');
          }
    ...
    public function actionUploadGeneral() {
        ...
        if ($modelHeader->load(Yii::$app->request->post())) {
            ...
            if (strcmp($highestColumn, "N") == 0 && $highestRow >= 4) {
                ...
                $rowCount = 2;
                $customTitle = array('SNo', 'CHECK/PF NUMBER', 'FORM FOUR INDEX NUMBER', 'FIRST NAME', 'MIDDLE NAME', 'SURNAME', 'DATE OF BIRTH(Year-Month-Day)', 'PLACE OF BIRTH(WARD)', 'MOBILE PHONE NUMBER', 'CURRENT NAME IF CHANGED', 'NAME OF INSTITUTION OF STUDY', 'NATIONAL IDENTIFICATION NUMBER(NIN)', 'BASIC SALARY(TZS)', 'GENDER', 'UPLOAD STATUS', 'FAILED REASON');
                $objPHPExcelOutput->getActiveSheet()->SetCellValue('A' . $rowCount, $customTitle[0]);
                $objPHPExcelOutput->getActiveSheet()->SetCellValue('B' . $rowCount, $customTitle[1]);
                $objPHPExcelOutput->getActiveSheet()->SetCellValue('C' . $rowCount, $customTitle[2]);
                $objPHPExcelOutput->getActiveSheet()->SetCellValue('D' . $rowCount, $customTitle[3]);
                ...
                for ($row = 4; $row <= $highestRow; ++$row) {
                   ...
                   //echo "<pre>";print_r($modelHeader->errors);
