...
//                . " return $('#education-under_sponsorship').val() == 1; }"],
 [['region_id'], 'required', 'when' => function ($model) {
                    return $model->country_id ==3;
                },
                'whenClient' => "function (attribute, value) { "
                . " return $('#education-country_id').val() ==3; }"],            
[['certificate_document'], 'required', 'message' => 'Please Attach  Certificate Document.', 'when' => function($model) {
            if($model->is_necta>1&&$model->is_necta<4){
        return  ($model->certificate_document) ? 0:1;
            }
        }, 'whenClient' => "function (attribute, value) {
            if ($('#education-is_necta').val() == '') {
               return 1;
            } else {
               return 0;
            }
...
[['sponsor_proof_document'], 'required', 'message' => 'Please Attach Sponsorship Document.', 'when' => function($model) {
        ...
        [['institution_name','level','avn_number','entry_year', 'programme_name', 'completion_year','under_sponsorship','country_id'], 'required', 'when' => function ($model) {
            ...
            [['application_id', 'level','registration_number', 'entry_year', 'completion_year', 'avn_number','institution_name','division','points','country_id','registration_number_old','region_id','are_you_post_f4'], 'safe'],
            ...
            [['entry_year','completion_year'], 'validateYears','skipOnEmpty' => false],
            [['level'], 'string'],
            ...
            [['sponsor_proof_document','certificate_document'], 'file', 'extensions'=>['pdf','jpg','png','jpeg']],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'are_you_post_f4'=>'Are you a Form 6 holder?'
        ];
    }
...
//        $pwd = Yii::$app->params['sel_password'];
   ...
   public function validateDateOfBirth($attribute, $params){
    ...
    // Maximum date today - 18 years
       // $year=-;    
    if($this->completion_year<$this->entry_year&&$this->level=="PRIMARY"){
 ...
 public static function getEducationDetails($application_id,$education_level){
  ...
  public static function getcheckeducation($level,$applicationId){
            $status=1;
          $models=\frontend\modules\application\models\Education::find()->where("level='{$level}' AND application_id='{$applicationId}' AND under_sponsorship<>0")->all();  
    ...
    public function getCountry()
    {
     ...
     public function checkprimary($level,$applicationId){
    $model=Education::find()->where(["level"=>$level,'application_id'=>$applicationId])->all();
    return $model;
   }
