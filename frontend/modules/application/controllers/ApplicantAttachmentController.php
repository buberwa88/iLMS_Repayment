    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           //$path = "uploads/applicant_attachments/".$filename;
            unlink($model->OldAttributes['attachment_path']);
            $path='applicant_attachment/'.$model->attachment_definition_id.'_'.$model->application_id.'_'.$model->applicant_attachment_id.'_'.date("Y") .'.'.$model->attachment_path->extension;
            
            if($model->attachment_path->saveAs($path)){
                //$model->verification_status=0;
                $model->comment=NULL;
                $model->other_description=NULL;
                $model->sponsor_address=NULL;
                $model->last_verified_by=NULL;
                $model->last_verified_at=NULL;
                $model->attachment_path =$path;
                $model->save(false);
                
                $uploaded =false;
                Yii::$app->getSession()->setFlash('success', 'Attachment uploaded Successfully');
                //print_r($model);
               // exit();
            }
        }
