    ...
    public function actionParentView() {
        ...
        if (Yii::$app->request->post()) {
            ...
            if ($modelNew->death_certificate_document != "") {
                 unlink($modelNew->OldAttributes['death_certificate_document']); 
                $modelNew->death_certificate_document->saveAs('applicant_attachment/death_certificate_document_'.$modelApplication->application_id.'_'.date("Y_m_d_h_i").'_'.$modelNew->sex.'.'.$modelNew->death_certificate_document->extension);
                $modelNew->death_certificate_document = 'applicant_attachment/death_certificate_document_'.$modelApplication->application_id.'_'.date("Y_m_d_h_i").'_'.$modelNew->sex.'.'.$modelNew->death_certificate_document->extension;
                ############## birth_certificate_document ###############
                
                 ApplicantAttachment::SaveAttachments($modelApplication->application_id,$docdea, $modelNew->death_certificate_document,1); 
            ...
            if ($modelNew->disability_document != "") {
                unlink($modelNew->OldAttributes['disability_document']); 
                $modelNew->disability_document->saveAs('applicant_attachment/disability_document_'.$modelApplication->application_id.'_'.date("Y_m_d_h_i").'_'.$modelNew->sex.'.'.$modelNew->disability_document->extension);
                $modelNew->disability_document = 'applicant_attachment/disability_document_'.$modelApplication->application_id.'_'.date("Y_m_d_h_i").'_'.$modelNew->sex.'.'.$modelNew->disability_document->extension;
                 ############## disability_document ###############
                ApplicantAttachment::SaveAttachments($modelApplication->application_id,$docdis, $modelNew->disability_document,1); 
            } else {
                $modelNew->disability_document = $modelNew->OldAttributes['disability_document'];
            }
            //check if exit
                $modelexit=  ApplicantAssociate::find()->where("type = 'PR' AND sex='{$modelNew->sex}' AND application_id='{$modelNew->application_id}' AND guarantor_type is NULL")->all();
    ...
    public function actionParentUpdate($id) {
        ...
        if ($modelNew->load(Yii::$app->request->post())) {
            ...
            if ($modelNew->is_parent_alive == "NO") {
                ...
                if ($modelNew->death_certificate_document != "") {
                      unlink($modelNew->OldAttributes['death_certificate_document']); 
                    $modelNew->death_certificate_document->saveAs('applicant_attachment/death_certificate_document_'.$modelNew->application_id.'_'.date("Y_m_d_h_i").'_'.$modelNew->sex.'.'.$modelNew->death_certificate_document->extension);
                    $modelNew->death_certificate_document = 'applicant_attachment/death_certificate_document_'.$modelNew->application_id.'_'.date("Y_m_d_h_i").'_'.$modelNew->sex.'.'.$modelNew->death_certificate_document->extension;
               
                    ############## birth_certificate_document ###############
                     
                       ApplicantAttachment::SaveAttachments($modelNew->application_id,$docdea, $modelNew->death_certificate_document,1); 
                ...
                if ($modelNew->disability_document != "") {
                unlink($modelNew->OldAttributes['disability_document']);
                $modelNew->disability_document->saveAs('applicant_attachment/disability_document_'.$modelNew->application_id.'_'.date("Y_m_d_h_i").'_'.$modelNew->sex.'.'.$modelNew->disability_document->extension);
                $modelNew->disability_document = 'applicant_attachment/disability_document_'.$modelNew->application_id.'_'.date("Y_m_d_h_i").'_'.$modelNew->sex.'.'.$modelNew->disability_document->extension;
                 ############## disability_document ###############
                    ############## disability_document ###############
                ApplicantAttachment::SaveAttachments($modelNew->application_id,$docdis, $modelNew->disability_document,1); 
                   ...
                   //  exit();
                    if (count($modelparent) > 0) {
                        ...
                        if($model->passport_photo != "") {
                             $link_data="applicant_attachment/profile/".$model->OldAttributes['passport_photo'];
                             // $linki_data="applicant_attachment/profile/". $model->passport_photo;
                             unlink($link_data); 
                            $model->passport_photo->saveAs('applicant_attachment/profile/photo_ga_'.$modelApplication->application_id.'_'.date("Y_h_i").'.'.$model->passport_photo->extension);
                            $model->file_path = 'applicant_attachment/profile/';
                            $model->passport_photo ="photo_ga_".$modelApplication->application_id.'_'.date("Y_h_i").'.'.$model->passport_photo->extension;
                              $link_data="applicant_attachment/profile/".$model->OldAttributes['passport_photo'];
                              $linki_data="applicant_attachment/profile/". $model->passport_photo;
                             //unlink($link_data); 
                             ApplicantAttachment::SaveAttachments($modelApplication->application_id,17,$linki_data,1); 
                        ...
                        if ($model->identification_document != "") {
                          unlink($model->OldAttributes['identification_document']); 
                            $model->identification_document->saveAs('applicant_attachment/document_ga_'.$modelApplication->application_id.'_'.date("Y_h_i").'.'.$model->identification_document->extension);
                            $model->identification_document = 'applicant_attachment/document_ga_'.$modelApplication->application_id.'_'.date("Y_h_i").'.'.$model->identification_document->extension;
                            ############## identification_document ###############
                         ApplicantAttachment::SaveAttachments($modelApplication->application_id,18, $model->identification_document,1); 
...
//                     exit();
                    if (count($modelparent) > 0 && $modelparent->firstname != "") {
                     ...
                     $modelinst=  \backend\modules\application\models\LearningInstitution::findOne($model->learning_institution_id);
                       //organization name
                        $model->organization_name = $modelinst->institution_name;
                        $model->firstname =  $modelinst->cp_firstname;
                        $model->middlename =  $modelinst->cp_middlename;
                        $model->surname =   $modelinst->cp_surname;
                        $model->phone_number =   $modelinst->phone_number;
                       // $model->postal_address =   $modelinst->postal_address;
                        $model->physical_address =  $modelinst->physical_address;
                        $model->email_address =   $modelinst->cp_email_address;
                        $model->sex='M';  
                        $model->ward_id =  $modelinst->ward_id;
                }
           
               $model->guaranteed_letter = UploadedFile::getInstance($model, 'guaranteed_letter');
              
               // $model->guaranteed_letter = UploadedFile::getInstance($model, 'guaranteed_letter');
                if ($model->guarantor_type==1&&$model->guaranteed_letter!="") {
                    unlink($model->OldAttributes['guaranteed_letter']); 
                    $model->guaranteed_letter->saveAs('applicant_attachment/ga_letter_'.$modelApplication->application_id.'_'.date("Y_h_i").'.'.$model->guaranteed_letter->extension);
                    $model->guaranteed_letter = 'applicant_attachment/ga_letter_'.$modelApplication->application_id.'_'.date("Y_h_i").'.'.$model->guaranteed_letter->extension;
                      ApplicantAttachment::SaveAttachments($modelApplication->application_id,19, $model->guaranteed_letter,1); 
            
                    } else {
                    $model->guaranteed_letter = $model->OldAttributes['guaranteed_letter'];
                }
