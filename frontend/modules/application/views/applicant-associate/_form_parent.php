...
for ($y = 1988; $y <= $yearmax; $y++) {
 ...
 if ($model->disability_status=="YES" && $model->disability_status != NULL&&$model->is_parent_alive=="YES") {
    ...
    if ($model->is_parent_alive=="NO" && $model->is_parent_alive != NULL) {
       //$nonenecta = "checked='checked'";
        echo '<style>
        .field-applicantassociate-death_certificate_document,.field-applicantassociate-death_certificate_number{
            display:block;
        }
    </style>';
    }
...
<div class="education-form">
   <?php
    ...
    'options' => ['accept' => 'image/*'],
        ...
        'browseLabel' =>  'Attach Death Certificate Document (required format .pdf,.jpg and .png only)'
    ]
]);
   echo Form::widget([
            'model' => $model,
