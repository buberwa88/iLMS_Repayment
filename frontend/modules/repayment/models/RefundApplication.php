...
//            [['updated_at'], 'required'],
    ...
    function validIdentity() {
        ///validate if it is a correct email of phone number
        if ($this->trustee_identity) {
            if (preg_match(' /^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,})$/', $this->trustee_identity)) {
                //returns true of it is correct email address format
                return TRUE;
...
//                echo $this->trustee_identity;
    ...
    static function getDetailsByIdentityNo($identity_no) {
        return self::find()
                        ->where('trustee_email="' . $identity_no . '" OR trustee_phone_number LIKE "%' . $identity_no . '%"')
                        ->orderBy('created_at DESC')
                        ->one();
    }

    function detailsExistByIdentityNo() {
        return self::find()
