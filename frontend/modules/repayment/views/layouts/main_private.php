...
<html lang="<?= Yii::$app->language ?>">
                    ...
                    \yiister\adminlte\widgets\Menu::widget(
                            [
                                "items" => [  
									...
									"url" => "#",
								"items" => [
                                ["label" => "Add/Upload Employees", "url" => Url::to(['/repayment/employed-beneficiary/add-uploademployee']), 'active' => ((Yii::$app->controller->id =='employed-beneficiary')&&(Yii::$app->controller->action->id=='add-uploademployee' || Yii::$app->controller->action->id=='create' ||Yii::$app->controller->action->id=='index-upload-employees')), "icon" => "users"],
                                    ["label" => "Loan Beneficiary", "url" => Url::to(['/repayment/employed-beneficiary/index-view-beneficiary']), 'active' => ((Yii::$app->controller->id =='employed-beneficiary'||Yii::$app->controller->id =='employer')&&(Yii::$app->controller->action->id=='index-view-beneficiary'||Yii::$app->controller->action->id=='update-salarysource'||Yii::$app->controller->action->id=='learning-institutions-codes')), "icon" => "users"],
							...
							[
                                ...
                                ["label" =>$label, "url" => Url::to(['/repayment/loan-repayment']), 'active' => (Yii::$app->controller->id =='loan-repayment'&&Yii::$app->controller->action->id=='index'||Yii::$app->controller->action->id=='generate-bill' || Yii::$app->controller->action->id=='confirm-payment' || Yii::$app->controller->action->id=='viewconfirmed-payment'), "icon" => "th",'visible' =>$salarySource !='1'],
    ...
    ["label" => "Successful Employees".$notificationNewAddedEmployeeFinal, "url" => Url::to(['/repayment/employed-beneficiary/notific-succ-uploadedemployee']), 'active' => (Yii::$app->controller->action->id == 'un-verified-uploaded-employees' || Yii::$app->controller->action->id == 'notific-succ-uploadedemployee')],
    ["label" => "Failed Employees".$notificationFailedEmployeeFinalReslt, "url" => Url::to(['/repayment/employed-beneficiary/notific-fail-uploadedemployee']), 'active' => (Yii::$app->controller->action->id == 'failed-uploaded-employees' || Yii::$app->controller->action->id == 'notific-fail-uploadedemployee')],
	...
	//["label" => "Contrl # Redirect", "url" => Url::to(['/repayment/loan-repayment/testgepg-controlnumber']), 'active' => (Yii::$app->controller->action->id == 'testgepg-controlnumber'&&Yii::$app->controller->id=='loan-repayment')],
	],
                        ],
