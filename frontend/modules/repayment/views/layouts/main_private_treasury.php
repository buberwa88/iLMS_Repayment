            ...
            <aside class="main-sidebar">
                <section class="sidebar">
                    ...
                    \yiister\adminlte\widgets\Menu::widget(
                            [
                                "items" => [
                                ["label" => "Manage Beneficiaries", "url" => Url::to(['/repayment/employed-beneficiary/index-treasury']), 'active' => ((Yii::$app->controller->id =='employed-beneficiary')&&(Yii::$app->controller->action->id=='index-treasury')), "icon" => "users"],								
							  [
									"label" => "Manage Loan",
									"icon" => "th",
									"url" => "#",
								"items" => [
                                ["label" => "Generate and Pay the Bill", "url" => Url::to(['/repayment/loan-repayment/index-treasury-bill']), 'active' => (Yii::$app->controller->id =='loan-repayment'&&Yii::$app->controller->action->id=='index-treasury-bill'||Yii::$app->controller->action->id=='generate-bill-treasury'||Yii::$app->controller->action->id=='confirm-payment-treasury'||Yii::$app->controller->action->id=='viewconfirmed-payment-treasury'||Yii::$app->controller->action->id=='view-treasury'), "icon" => "th"],
                                 ["label" => "Payments", "url" => Url::to(['/repayment/loan-repayment-detail/bills-payments-treasury']), 'active' => (Yii::$app->controller->id =='loan-repayment-detail'), "icon" => "th"],
                                 ["label" => "Receipts", "url" => Url::to(['/repayment/loan-repayment/receipt-treasury']), 'active' => (Yii::$app->controller->id =='loan-repayment'&&Yii::$app->controller->action->id=='receipt-treasury'), "icon" => "money"],									
                                ],
                            ],
									//["label" => "My Account", "url" => Url::to(['/repayment/employer/view','id' =>$employerID]), 'active' => ((Yii::$app->controller->action->id == 'view' || Yii::$app->controller->action->id == 'update-information')&&Yii::$app->controller->id=='employer'), "icon" => "users"],
                                ],
                                'encodeLabels' => false,
                            ]
                    )
