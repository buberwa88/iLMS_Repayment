...
//end set session
    ...
    public function actionCreateRefundf4education2() {
        $this->layout = "main_public";
//$model = new \frontend\modules\repayment\models\RefundClaimant();
//set session
        $session = Yii::$app->session;
...
//end set session
    ...
    public function actionCreateTertiary1() {
        $this->layout = "main_public";
        $model = new \frontend\modules\repayment\models\RefundClaimantEducationHistory();
        $model->scenario = 'refundTresuryEducation';
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
...
//end set session
    ...
    public function actionContactDetailsPreview1($id) {
        $this->layout = "main_public";
        $session = Yii::$app->session;
        $refund_application_id = $session->get('refund_application_id');
...
//            var_dump($model->attributes);
            if ($model->validate()) {
//echo 'Take to efund View';
                $refund = \frontend\modules\repayment\models\RefundApplication::getDetailsByApplicationNo($model->application_number);
// var_dump($refund->attributes);
                echo $refund->refund_application_id;
                Yii::$app->session->set('claimant_id', $refund->refund_claimant_id);
// exit;
    ...
    public function actionClaimantRefunds() {
        ...
        if (Yii::$app->session->has('claimant_id')) {
            $claimant_id = Yii::$app->session->get('claimant_id');
